// app/components/auth/SocialAuthButtons.jsx
'use client';
 ...
 * Props:
 * - onSuccess: Function called after successful social login
...
export default function SocialAuthButtons({ onSuccess, onError, disabled = false }) {
  ...
  const handleGoogleSignIn = async () => {
    if (loading.google || disabled) return;
  ...
  const handleFacebookSignIn = async () => {
    if (loading.facebook || disabled) return;
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        ...
        className="w-full flex items-center justify-center px-4 py-2 border border-gray-300 rounded-lg shadow-sm bg-white text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
      >
        {loading.google ? (
